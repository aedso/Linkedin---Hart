...
// NOTE: In production (Vercel), replace these with env variables:
// const SUPA_URL = process.env.NEXT_PUBLIC_SUPABASE_URL
// const SUPA_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY
// And use localStorage for session persistence.
...
const fmtMonth = iso => {
  ...
  const [y,mo]=iso.split("-"); return `${PT[+mo-1]}/${y.slice(2)}`;
};
...
function parseEngagement(ws) {
  return XLSX.utils.sheet_to_json(ws,{header:1}).slice(1).map(r=>{
    const d=toIso(r[0]); return d?{date:d,impressions:+r[1]||0,engagements:+r[2]||0}:null;
  }).filter(Boolean);
}
...
function createClient(url,anonKey,token) {
  const hdrs={"apikey":anonKey,"Authorization":`Bearer ${token||anonKey}`,"Content-Type":"application/json"};
  const req=async(path,opts={})=>{
    const r=await fetch(`${url}${path}`,{...opts,headers:{...hdrs,...(opts.headers||{})}});
    const text=await r.text();
    let data; try{data=JSON.parse(text);}catch{data=text;}
    if(!r.ok) throw new Error(typeof data==="object"?JSON.stringify(data):String(text));
    return data;
  };
  return {
    auth:{
      signIn:(email,pw)=>fetch(`${url}/auth/v1/token?grant_type=password`,{method:"POST",headers:{"apikey":anonKey,"Content-Type":"application/json"},body:JSON.stringify({email,password:pw})}).then(r=>r.json()),
      signOut:()=>fetch(`${url}/auth/v1/logout`,{method:"POST",headers:{"apikey":anonKey,"Authorization":`Bearer ${token}`}}).catch(()=>{}),
    },
    from:table=>({
      select:(cols="*",qs="")=>req(`/rest/v1/${table}?select=${cols}${qs}`),
      upsert:(data,onConflict)=>req(`/rest/v1/${table}${onConflict?`?on_conflict=${onConflict}`:""}`,{
        method:"POST",headers:{"Prefer":"resolution=merge-duplicates,return=minimal"},
        body:JSON.stringify(Array.isArray(data)?data:[data])
      }),
      insert:data=>req(`/rest/v1/${table}`,{
...
async function saveToSupabase(client,parsed,existingProfiles) {
  // 1. Find or create profile (dedup by name, case-insensitive)
  ...
  // 3. Engagement daily — dedup on (profile_id, date). Batches of 200.
...
// ── UI Primitives ─────────────────────────────────────────────────────────────
const S={ card:{background:"#fff",borderRadius:12,boxShadow:"0 1px 4px rgba(0,0,0,0.07)"} };
...
const Btn=({children,onClick,disabled,variant="primary",small,full})=>(
  <button onClick={onClick} disabled={disabled} style={{
    padding:small?"5px 14px":"11px 22px",width:full?"100%":"auto",
    background:disabled?"#E2E8F0":variant==="primary"?"linear-gradient(90deg,#0077B5,#005885)":variant==="ghost"?"transparent":"#F1F5F9",
    color:disabled?"#94A3B8":variant==="primary"?"#fff":"#334155",
    border:variant==="ghost"?"1px solid #E2E8F0":"none",
    borderRadius:8,fontSize:small?12:14,fontWeight:600,cursor:disabled?"not-allowed":"pointer",transition:"all 0.15s",whiteSpace:"nowrap"
  }}>{children}</button>
);

const StatCard=({label,value,sub,accent})=>(
  <div style={{...S.card,padding:"18px 20px",borderLeft:`4px solid ${accent}`}}>
...
const ConfigScreen=({onSave})=>{
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#EFF6FF,#F8FAFC)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,sans-serif",padding:24}}>
      <div style={{background:"#fff",borderRadius:20,padding:40,maxWidth:460,width:"100%",boxShadow:"0 8px 40px rgba(0,119,181,0.12)"}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          ...
          <div style={{fontSize:20,fontWeight:700,color:"#0F172A",marginBottom:4}}>Conectar Supabase</div>
          <div style={{fontSize:13,color:"#94A3B8",lineHeight:1.6}}>Cole as credenciais do seu projeto Supabase.<br/>Configuração salva — uma vez só por browser.</div>
        </div>
        <Input label="Project URL" value={url} onChange={setUrl} placeholder="https://xxxxxxxx.supabase.co" autoFocus mono />
        <Input label="Anon Key (public)" value={key} onChange={setKey} placeholder="eyJhbGciOiJIUzI1NiIs..." mono />
        <Btn full onClick={()=>onSave(url.trim(),key.trim())} disabled={!url||!key}>Conectar →</Btn>
        <div style={{marginTop:18,background:"#F8FAFC",borderRadius:8,padding:"14px 16px",fontSize:12,color:"#64748B",lineHeight:1.8}}>
          <strong style={{color:"#334155"}}>Onde encontrar no Supabase:</strong><br/>
...
// ── Auth Screen ───────────────────────────────────────────────────────────────
const AuthScreen=({client,onLogin,onReset})=>{
  ...
  const login=async()=>{
    ...
    else setErr(data.error_description||data.msg||"Credenciais inválidas. Verifique e-mail e senha.");
  };
  const onKey=e=>{ if(e.key==="Enter"&&email&&pass) login(); };
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#EFF6FF,#F8FAFC)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,sans-serif",padding:24}}>
      <div style={{background:"#fff",borderRadius:20,padding:40,maxWidth:380,width:"100%",boxShadow:"0 8px 40px rgba(0,119,181,0.12)"}}>
        ...
        <div style={{marginTop:16,textAlign:"center"}}>
          <button onClick={onReset} style={{background:"none",border:"none",color:"#94A3B8",fontSize:11,cursor:"pointer"}}>Reconfigurar conexão</button>
        </div>
      </div>
    </div>
  );
...
// ── Upload Modal ──────────────────────────────────────────────────────────────
const UploadModal=({client,profiles,onClose,onDone})=>{
  ...
  const addFiles=useCallback(fs=>{
    const arr=Array.from(fs).map(f=>({file:f,name:detectName(f.name)}));
    setFiles(p=>[...p,...arr]);
  },[]);
  ...
  const process=async()=>{
    ...
    try{ const fresh=await client.from("li_profiles").select("*"); if(Array.isArray(fresh)) allProfs=fresh; }catch{}
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:20,fontFamily:"system-ui,sans-serif"}}>
      <div style={{background:"#fff",borderRadius:20,padding:32,maxWidth:520,width:"100%",maxHeight:"90vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.25)"}}>
        ...
        {files.map((item,i)=>{
          ...
          const isDone=st==="done"; const isLoading=st==="uploading";
          return(
            <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",background:"#F8FAFC",borderRadius:8,marginBottom:6,border:`1px solid ${isErr?"#FEE2E2":isDone?"#DCFCE7":"#F1F5F9"}`}}>
              ...
              <div style={{fontSize:11,fontWeight:500,flexShrink:0,color:isDone?"#16A34A":isLoading?"#0077B5":isErr?"#DC2626":"#94A3B8"}}>
                {isDone?"✓ Salvo":isLoading?"⏳...":isErr?"✗ Erro":"—"}
              </div>
        ...
        <div style={{margin:"10px 0 18px",background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:8,padding:"8px 14px",fontSize:11,color:"#166534",display:"flex",gap:6}}>
          <span>✓</span>
          <span>Deduplicação automática — fazer upload do mesmo arquivo mais de uma vez é seguro. Dados existentes são atualizados, não duplicados.</span>
        </div>

        <div style={{display:"flex",gap:8}}>
          <Btn onClick={process} disabled={!files.length||running}>{running?`Salvando...`:`Salvar ${files.length} arquivo${files.length!==1?"s":""} →`}</Btn>
          <Btn variant="ghost" onClick={onClose} disabled={running}>Cancelar</Btn>
        </div>
      </div>
    </div>
  );
...
const Dashboard=({client,onLogout})=>{
  ...
  const [data,setData]=useState({eng:[],fol:[],posts:[],disc:[],demo:[]});
  ...
  const loadAll=useCallback(async()=>{
    ...
    try{
      ...
      setProfiles(profs||[]); setData({eng:eng||[],fol:fol||[],posts:posts||[],disc:disc||[],demo:demo||[]});
  ...
  const totals=useMemo(()=>{
    const disc=(data.disc||[]).filter(d=>selIds.has(d.profile_id));
    const eng=(data.eng||[]).filter(d=>selIds.has(d.profile_id));
    const latest={};
  ...
  },[data.disc,data.eng,selIds]);

  const impChart=useMemo(()=>{
    const months=new Set((data.eng||[]).map(d=>d.date?.slice(0,7)).filter(Boolean));
    const byP={};
    profiles.forEach(p=>{ byP[p.id]={}; (data.eng||[]).filter(d=>d.profile_id===p.id).forEach(d=>{ const mo=d.date?.slice(0,7); if(mo) byP[p.id][mo]=(byP[p.id][mo]||0)+d.impressions; }); });
  ...
  const folChart=useMemo(()=>{
    const months=new Set((data.fol||[]).map(d=>d.date?.slice(0,7)).filter(Boolean));
    const byP={};
    profiles.forEach(p=>{ byP[p.id]={}; (data.fol||[]).filter(d=>d.profile_id===p.id).forEach(d=>{ const mo=d.date?.slice(0,7); if(mo) byP[p.id][mo]=(byP[p.id][mo]||0)+d.new_followers; }); });
    return Array.from(months).sort().map(mo=>{ const r={month:fmtMonth(mo)}; profiles.forEach(p=>{r[p.name]=byP[p.id]?.[mo]||0;}); return r; });
  },[data.fol,profiles]);

  const topByEng=useMemo(()=>
    (data.posts||[]).filter(p=>selIds.has(p.profile_id)).sort((a,b)=>b.engagements-a.engagements).slice(0,10)
  ...
  [data.posts,selIds,profiles]);

  const topByImp=useMemo(()=>
    (data.posts||[]).filter(p=>selIds.has(p.profile_id)).sort((a,b)=>b.impressions-a.impressions).slice(0,10)
      .map(p=>({...p,profileName:profiles.find(x=>x.id===p.profile_id)?.name||"?",color:COLORS[profiles.findIndex(x=>x.id===p.profile_id)%COLORS.length]})),
  [data.posts,selIds,profiles]);
  ...
  const demoItems=useMemo(()=>{
    ...
    const r={};
    (data.demo||[]).filter(d=>d.profile_id===demProfile.id).forEach(item=>{ if(!r[item.category]) r[item.category]=[]; r[item.category].push({label:item.value,pct:item.percentage}); });
    return r;
  },[data.demo,demProfile]);

  if(loading) return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,sans-serif",color:"#94A3B8",background:"#F1F5F9"}}>
      <div style={{textAlign:"center"}}><div style={{fontSize:40,marginBottom:12}}>⏳</div><div style={{fontSize:14}}>Carregando dados...</div></div>
    </div>
  );
  ...
  if(!profiles.length) return(
    <div style={{minHeight:"100vh",background:"#F1F5F9",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,sans-serif"}}>
      <div style={{background:"#fff",borderRadius:20,padding:48,maxWidth:420,textAlign:"center",boxShadow:"0 4px 20px rgba(0,0,0,0.08)"}}>
        <div style={{fontSize:56,marginBottom:16}}>📊</div>
        <div style={{fontSize:18,fontWeight:700,color:"#0F172A",marginBottom:8}}>Tudo pronto!</div>
        <div style={{fontSize:13,color:"#94A3B8",marginBottom:24,lineHeight:1.7}}>Faça o upload dos .xlsx exportados do LinkedIn<br/>para popular o dashboard</div>
        <Btn onClick={()=>setShowUpload(true)}>Primeiro upload →</Btn>
        {showUpload&&<UploadModal client={client} profiles={[]} onClose={()=>setShowUpload(false)} onDone={()=>{setShowUpload(false);loadAll();}}/>}
      </div>
    </div>
  );
  ...
  return(
    <div style={{minHeight:"100vh",background:"#F1F5F9",fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{background:"#fff",borderBottom:"1px solid #E2E8F0",position:"sticky",top:0,zIndex:50,boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 20px",display:"flex",alignItems:"stretch",height:52,gap:4}}>
          ...
          <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:5,flexShrink:0}}>
            ...
            <button onClick={()=>setShowUpload(true)} style={{padding:"4px 12px",borderRadius:20,background:"#0077B5",color:"#fff",border:"none",fontSize:12,fontWeight:600,cursor:"pointer"}}>+ Upload</button>
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:24}}>
        ...
        {tab==="overview"&&(
          <>
            ...
            {profiles.length>1&&(
              <div style={{...S.card,padding:20,marginBottom:18,overflowX:"auto"}}>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,minWidth:640}}>
                  ...
                  <tbody>
                    {profiles.map((p,i)=>{
                      const disc=(data.disc||[]).filter(d=>d.profile_id===p.id);
                      const lat=disc.reduce((a,b)=>(!a||b.snapshot_date>a.snapshot_date)?b:a,null)||{};
                      const eng=(data.eng||[]).filter(d=>d.profile_id===p.id);
                      const totI=eng.reduce((s,d)=>s+d.impressions,0);
                      const totE=eng.reduce((s,d)=>s+d.engagements,0);
                      const er=totI>0?((totE/totI)*100).toFixed(2):"0.00";
                      const pP=(data.posts||[]).filter(d=>d.profile_id===p.id);
      ...
      {showUpload&&<UploadModal client={client} profiles={profiles} onClose={()=>setShowUpload(false)} onDone={()=>{setShowUpload(false);loadAll();}}/>}
    </div>
  );
};
...
export default function App() {
  // In production (Vercel), persist config and session in localStorage:
  // const [config,setConfig]=useState(()=>{ try{return JSON.parse(localStorage.getItem("hart_config")||"null");}catch{return null;} });
  // const [session,setSession]=useState(()=>{ try{return JSON.parse(localStorage.getItem("hart_session")||"null");}catch{return null;} });
  const [config,setConfig]=useState(null);
  const [session,setSession]=useState(null);

  const saveConfig=(url,key)=>setConfig({url,key});
  const handleLogin=sess=>setSession(sess);
  const handleLogout=async()=>{ if(config&&session) createClient(config.url,config.key,session.access_token).auth.signOut().catch(()=>{}); setSession(null); };
  const resetConfig=()=>{ setConfig(null); setSession(null); };

  if(!config) return <ConfigScreen onSave={saveConfig}/>;
  const client=createClient(config.url,config.key,session?.access_token);
  if(!session) return <AuthScreen client={client} onLogin={handleLogin} onReset={resetConfig}/>;
